...
4. Don't start the server yet - do Step 4 (generate) first so there's a room to `/connect` to.
...
> **The old external connector is retired.** `ArkConnector.zip` is no longer part of a release -
> the plugin's built-in client does everything it did, including the `randomize_dino_spawns`
> Game.ini patch (via `/confirm`). Nothing to run alongside the server. The Python source still
> lives in `connector/` in the repo for reference, but it isn't supported or released.
...
3. **Add yamls:** put your ARK yaml in `Players\`. Start from the example `ark.yaml` bundled in the
   apworld - set `name:` to your slot (the name you'll type in `/connect`). Each friend drops in their own
...
6. Note that `host:port` (and password, if any) - you'll pass it to `/connect` in game chat.
...
5. **Connect to the room from game chat** (once spawned in):
   ...
   restart it reconnects by itself.
...
3. Connect each player - easiest is in game chat: each player **spawns in fully on their
   ...
   can't read your survivor name yet, wait a moment and retry). Each player gets their own mailbox
   under the plugin's `ipc\<SurvivorName>\` - nothing to configure per player.
...
If more than one ARK yaml uses `randomize_dino_spawns`, only ONE player should run `/confirm`
(there's a single Game.ini; the last writer would win).
...
The plugin's AP client makes an outbound-only connection (to the AP room) - it never needs a
forwarded port itself, in solo or multiplayer mode. If you only ever transfer characters locally (no remote
...
| `dossier_checks` | `232` | How many explorer-note checks to include (max = all Island notes). |
...
| `randomize_dino_spawns` | `off` | Fully randomize which species live in which biome - each zone's roster is replaced by a seeded hand, every species still spawns somewhere (`grouped` = habitat-appropriate, `chaos` = anything anywhere). `/confirm` writes the Game.ini lines and restarts the server. |
...
> **`randomize_dino_spawns`:** set it in the yaml you **generate from**, with the current
> `ark_ase.apworld` installed (older apworlds don't emit it). After you `/connect`, the plugin
> tells you to type **`/confirm`** - that writes the spawn lines into `Game.ini`, saves, and
> restarts the server with a forced wild-dino wipe so the new rosters populate immediately.
> One time per seed. (Prefer doing it by hand? The lines are also written to
> `ipc\game_ini_fragment.txt`.)
...
On reconnect AP resends everything and the plugin dedups by item index, so you only ever reset the
plugin + world.
